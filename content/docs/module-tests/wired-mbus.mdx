...
On the **A0 Baseboard**, the TSS721ADR enables **direct physical connection** to wired M-Bus meters and acts as the **physical layer interface** between the meters and the A0 board.

This module allows the A0 board to collect meter data from the Baseboard and forward it to Magistrala via protocols like MQTT, CoAP, HTTP, and others.
...
## Role of the TSS721ADR on the A0 Board
...
A0 communicates with the TSS721ADR using the **UART interface**, while the TSS721ADR interfaces with the **M-Bus lines (MBUS+ / MBUS−)**.
...
- A0 Baseboard powered
- A0, or a microcontroller of your choice powered and programmed
...
- Serial logging enabled on the A0 or microcontroller of your choice
